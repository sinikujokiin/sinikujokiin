          ...
          <div class="row">
            <div class="col-12">
              <div class="form-group">
                <label for="text">Nama Team</label>
              ...
              <div class="form-group">
                ...
                <span class="jabatan_error text-danger"></span>
              </div>
              <div class="form-group">
...
<script>
  ...
  function loadProduct()
  {
    dt = $("#dt-kategori").DataTable({
      ...
      "autoWidth": false,
        ...
        "destroy":true,
        "ajax": {
          "url":base_url+"cms/get-data-team",
          "type": "POST",
        },
        "columnDefs": [
        {
  ...
  $('.btn-submit').click(function(e){
    ...
    $.ajax({
      url:base_url+'cms/save-team/'+aksi,
      dataType:'json',
      type:'POST',
      data: formData,
      contentType: false,
  ...
  function ButtonEdit(id)
  {
    ...
    $.ajax({
      url:base_url+'cms/get-data-team/'+id,
      dataType:'json',
      success: (response) => {
        $.each(response.data, function(key, value) {
          if (key == 'image') {
            if (value) {
              $("#show-image").html(`<img src="${base_url}uploads/team/${value}" width="150px" alt="Gambar ${response.data.nama}">`);
            }else{
              $("#show-image").html(``);
            }
          }else{
  ...
  {
      $.ajax({
        url:base_url+'cms/update-status-team/'+id,
        type:'post',
        dataType:'json',
        success: (response) => {
          sukses(response.alert);
  ...
  function ButtonDelete(id)
  {
    Swal.fire({ 
      ...
      if (result.value) {
          $.ajax({
            url:base_url+'cms/delete-team/'+id,
            type:'post',
            dataType:'json',
            success: (response) => {
              sukses(response.alert);
