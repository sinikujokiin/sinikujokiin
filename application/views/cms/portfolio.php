          ...
          <div class="row">
            <div class="col-12">
              <div class="form-group">
                <label for="text">Kategori Portfolio</label>
                <input type="hidden" name="id_portfolio" id="id_portfolio" value="">
                <select class="form-control select2-tags" name="kategori_portfolio" id="kategori_portfolio">
                  <?php foreach ($kategori as $value): ?>
                    <option value="<?= $value['kategori_portfolio'] ?>"><?= $value['kategori_portfolio'] ?></option>
                  <?php endforeach ?>
                </select>
...
<script>
  ...
  function loadProduct()
  {
    dt = $("#dt-kategori").DataTable({
      ...
      "autoWidth": false,
        ...
        "destroy":true,
        "ajax": {
          "url":base_url+"cms/get-data-portfolio",
          "type": "POST",
        },
        "columnDefs": [
        {
  ...
  $('.btn-submit').click(function(e){
    ...
    $.ajax({
      url:base_url+'cms/save-portfolio/'+aksi,
      dataType:'json',
      type:'POST',
      data: formData,
      contentType: false,
  ...
  function ButtonEdit(id)
  {
    ...
    $.ajax({
      url:base_url+'cms/get-data-portfolio/'+id,
      dataType:'json',
      success: (response) => {
        $.each(response.data, function(key, value) {
          if (key == 'image') {
            if (value) {
              $("#show-image").html(`<img src="${base_url}uploads/portfolio/${value}" width="150px" alt="Gambar ${response.data.nama}">`);
            }else{
              $("#show-image").html(``);
            }
          }else{
  ...
  {
      $.ajax({
        url:base_url+'cms/update-status-portfolio/'+id,
        type:'post',
        dataType:'json',
        success: (response) => {
          sukses(response.alert);
  ...
  function ButtonDelete(id)
  {
    Swal.fire({ 
      ...
      if (result.value) {
          $.ajax({
            url:base_url+'cms/delete-portfolio/'+id,
            type:'post',
            dataType:'json',
            success: (response) => {
              sukses(response.alert);
