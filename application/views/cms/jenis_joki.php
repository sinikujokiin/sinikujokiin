          ...
          <div class="row">
            <div class="col-12">
              <div class="form-group">
                ...
                <input type="hidden" name="id_testimoni_chat" id="id_testimoni_chat" value="">
                <input type="text" class="form-control" name="nama_jenis" id="nama_jenis" value="">
                <span class="nama_jenis_error text-danger"></span>
              </div>
              <div class="form-group">
...
<script>
  ...
  function loadProduct()
  {
    dt = $("#dt-kategori").DataTable({
      ...
      "autoWidth": false,
        ...
        "destroy":true,
        "ajax": {
          "url":base_url+"cms/get-data-jenis-joki",
          "type": "POST",
        },
        "columnDefs": [
        {
  ...
  $('.btn-submit').click(function(e){
    ...
    $.ajax({
      url:base_url+'cms/save-jenis-joki/'+aksi,
      dataType:'json',
      type:'POST',
      data: formData,
      contentType: false,
  ...
  function ButtonEdit(id)
  {
    ...
    $.ajax({
      url:base_url+'cms/get-data-jenis-joki/'+id,
      dataType:'json',
      success: (response) => {
        $.each(response.data, function(key, value) {
          if (key == 'image') {
            if (value) {
              $("#show-image").html(`<img src="${base_url}uploads/testimoni_chat/${value}" width="150px" alt="Gambar ${response.data.nama}">`);
            }else{
              $("#show-image").html(``);
            }
          }else{
  ...
  {
      $.ajax({
        url:base_url+'cms/update-status-jenis-joki/'+id,
        type:'post',
        dataType:'json',
        success: (response) => {
          sukses(response.alert);
  ...
  function ButtonDelete(id)
  {
    Swal.fire({ 
      ...
      if (result.value) {
          $.ajax({
            url:base_url+'cms/delete-jenis-joki/'+id,
            type:'post',
            dataType:'json',
            success: (response) => {
              sukses(response.alert);
